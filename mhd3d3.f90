...
!  2. 3d mpi domain decomposition along x, y and z
...
!..Program name. Length = exact length of characters used
   character(6), parameter :: progname = 'mhd3d3'
...
!..IMPORTANT: the length of the following strings must equal the number of characters
   character(28) :: outdir = '/global/scratch/bzhu/mhd3d3/'
!   character(2) :: outdir = './'
...
!..These are used for creating and using subcommunicators
   integer :: COMM_X,COMM_Y,COMM_Z,remain(3)
   integer :: xID,yID,zID,xcoord,ycoord,zcoord
!..Rank of process to the left and right (along x).
   integer :: left, right
!..Rank of process in up and down (along y).
   integer :: up, down
!..Rank of process in front and back (along z).
   integer :: front, back
...
!..Add here the namelists for parameters (first declared above) from input file:
   ...
   namelist/grid/nx0G,ny0G,nz0G,xprocs,yprocs,zprocs
   ...
   if (myrank.eq.0) then
      ...
      open(unit=10,file="mhd3d3.in",status="old")
      read (10,dom)
      read (10,grid)
      read (10,comset)
      read (10,list)
...
!*********************************************
   ...
   implicit none
   allocate(x(nxG+1,nyG+1,nzG+1),y(nxG+1,nyG+1,nzG+1),z(nxG+1,nyG+1,nzG+1))
!   allocate(xc(nxG,nyG,nzG),yc(nxG,nyG,nzG),zc(nxG,nyG,nzG))
!   allocate(dx(nxG,nyG,nzG),dy(nxG,nyG,nzG),dz(nxG,nyG,nzG))
!   allocate(xi(nxG+1,nyG,nzG),yi(nxG+1,nyG,nzG),zi(nxG+1,nyG,nzG))
!   allocate(xj(nxG,nyG+1,nzG),yj(nxG,nyG+1,nzG),zj(nxG,nyG+1,nzG))
!   allocate(xk(nxG,nyG,nzG+1),yk(nxG,nyG,nzG+1),zk(nxG,nyG,nzG+1))
...
!************************************
   subroutine fillz_center(aa,lnx,lny,lnz,zbc)
!..Fills the nord ghost cells in z (ng on either side) 
!  of an input array with unfilled z-ghost cells: aa(:,:,k) 
!  with k=1, ..., ng, nz-ng+1, ..., nz
   real*8, allocatable :: aa(:,:,:)
   real*8, allocatable :: sbufz(:,:,:),rbufz(:,:,:)
   integer :: k,lnx,lny,lnz,zbc,ncount

!..each proc has both of the needed ghost cell data for its neighbors:
   allocate(sbufz(lnx,lny,ng),rbufz(lnx,lny,ng))
   ncount=lnx*lny*ng

!..Send front most cells to the front
   sbufz=aa(:,:,lnz-nord+1:lnz-ng)
   call MPI_SENDRECV(sbufz,ncount,MPI_DOUBLE_PRECISION,front,0, &
                     rbufz,ncount,MPI_DOUBLE_PRECISION,back,0,COMM_Z,istat,ierr)
   aa(:,:,1:ng)=rbufz

!..Send back most cells to the back
   sbufz=aa(:,:,ng+1:nord)
   call MPI_SENDRECV(sbufz,ncount,MPI_DOUBLE_PRECISION,back,1, &
                     rbufz,ncount,MPI_DOUBLE_PRECISION,front,1,COMM_Z,istat,ierr)
   aa(:,:,lnz-ng+1:lnz)=rbufz

   deallocate(sbufz,rbufz)

!..Fill the first and last process (along z) separately and enforce the
!..corresponding symmetry
   if (zID .eq. 0) then ! back z-boundary
      if (zbc .eq. 0) then ! aa is even about z=-Lz/2
         forall(k=1:ng) aa(:,:,k)=aa(:,:,nord+1-k)
      elseif (zbc .eq. 1) then ! aa is odd about z=-Lz/2
         forall(k=1:ng) aa(:,:,k)=-aa(:,:,nord+1-k)
!     could continue define zbc=2,3 and so on, eg, extrapolation, perodic ...
      endif
   elseif (zID .eq. zprocs-1) then ! front z-boundary
      if (zbc .eq. 0) then ! aa is even about z=Lz/2
         forall(k=1:ng) aa(:,:,lnz+1-k)=aa(:,:,lnz-nord+k)
      elseif(zbc .eq. 1) then
         forall(k=1:ng) aa(:,:,lnz+1-k)=-aa(:,:,lnz-nord+k)
      endif
   endif

   end subroutine fillz_center
!************************************
...
!************************************
   subroutine fillz_face(aa,lnx,lny,lnz,zbc)
!..Fills the nord ghost cells in z (ng on either side) 
!  of an input array with unfilled z-ghost cells: aa(:,:,k) 
!  with k=1, ..., ng, nz-ng+1, ..., nz
   real*8, allocatable :: aa(:,:,:)
...
!..Send front most cells to the front
   sbufz=aa(:,:,lnz-nord:lnz-ng-1)
   call MPI_SENDRECV(sbufz,ncount,MPI_DOUBLE_PRECISION,front,0, &
                     rbufz,ncount,MPI_DOUBLE_PRECISION,back,0,COMM_Z,istat,ierr)
   aa(:,:,1:ng)=rbufz

!..Send back most cells to the back
   sbufz=aa(:,:,ng+2:nord+1)
   call MPI_SENDRECV(sbufz,ncount,MPI_DOUBLE_PRECISION,back,1, &
                     rbufz,ncount,MPI_DOUBLE_PRECISION,front,1,COMM_Z,istat,ierr)
   aa(:,:,lnz-ng+1:lnz)=rbufz
...
!..corresponding symmetry
   if (zID .eq. 0) then ! back z-boundary
      if (zbc .eq. 0) then ! aa is even about z=-Lz/2
         forall(k=1:ng) aa(:,:,k)=aa(:,:,nord+2-k)
      elseif (zbc .eq. 1) then ! aa is odd about z=-Lz/2
         forall(k=1:ng) aa(:,:,k)=-aa(:,:,nord+2-k)
!     could continue define zbc=2,3 and so on, eg, extrapolation, perodic ...
      endif
   elseif (zID .eq. zprocs-1) then ! front z-boundary
      if (zbc .eq. 0) then ! aa is even about z=Lz/2
         forall(k=1:ng) aa(:,:,nz+2-k)=aa(:,:,nz0+k)
      elseif(zbc .eq. 1) then
         forall(k=1:ng) aa(:,:,nz+2-k)=-aa(:,:,nz0+k)
      endif
   endif

   end subroutine fillz_face
!************************************
...
!************************************
   ...
   call fillx_center(vx,nx,ny,nz,0)
   ...
   call filly_center(vy,nx,ny,nz,0)
   call filly_center(vz,nx,ny,nz,0)

   call fillz_center(rho,nx,ny,nz,0)
   call fillz_center(p,nx,ny,nz,0)
   call fillz_center(vx,nx,ny,nz,0)
   call fillz_center(vy,nx,ny,nz,0)
   call fillz_center(vz,nx,ny,nz,0)
...
!************************************
   ...
   call fillz_center(bi,nx+1,ny,nz,0)
   call fillz_center(bj,nx,ny+1,nz,0)
   call fillz_face(bk,nx,ny,nz+1,0)
...
!*****************************************
   subroutine interp2_pdm(f,fi,dir,reconop)
...
!   print*,tau
   ...
   call MPI_REDUCE(tau,gtau,1,MPI_DOUBLE_PRECISION,MPI_MIN,0,MPI_COMM_WORLD,ierr)
   tau=gtau
   call MPI_BCAST(tau,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
...
!..apply Alfven resistivity
   if (kkk.eq.1) then
      forall(i=1:ii,j=1:jj,k=1:kk) eta(i,j,k)= .25* &
            (vf(i+ng,j+ng-1,k+ng-1)+vf(i+ng,j+ng,k+ng-1)+ &
             vf(i+ng,j+ng,k+ng)+vf(i+ng,j+ng-1,k+ng))
   elseif(kkk.eq.2) then
      forall(i=1:ii,j=1:jj,k=1:kk) eta(i,j,k)= .25* &
            (vf(i+ng-1,j+ng,k+ng-1)+vf(i+ng-1,j+ng,k+ng)+ &
             vf(i+ng,j+ng,k+ng)+vf(i+ng,j+ng,k+ng-1))
   elseif(kkk.eq.3) then
      forall(i=1:ii,j=1:jj,k=1:kk) eta(i,j,k)= .25* &
            (vf(i+ng-1,j+ng-1,k+ng)+vf(i+ng,j+ng-1,k+ng)+ &
...
!.....first get interped velocity vi
      call interp2_pdm(lv,vi,dir,reconop)
      if(dir.eq.1) then
...
!   call createh5_2D('bz',2,nframes+1)
   ...
   call createh5_3D('rho',2,nframes+1)
   call createh5_3D('vx',2,nframes+1)
   call createh5_3D('vy',2,nframes+1)
   call createh5_3D('vz',2,nframes+1)
   call createh5_3D('p',2,nframes+1)
   call createh5_3D('bx',2,nframes+1)
   call createh5_3D('by',2,nframes+1)
   call createh5_3D('bz',2,nframes+1)
...
!*********************************************************
!..Create file dataname.h5, with dataset /data (identified by dsetid) with
!..dimensions [nx0G,ny0G,nz0G,nframes+1] (ONE FOR EACH ITERATION + 1 INITIAL)
   subroutine createh5_3D(dataname,prec,frames)
   implicit none
   character(len=*), intent(in) :: dataname
   integer(HID_T):: dsetid, fid, dspaceid
   integer, intent(in) :: prec,frames
...
!...Create a new file using DEFAULT PROPERTIES
   call h5fcreate_f(trim(dirname)//trim(dataname)//'.h5',H5F_ACC_TRUNC_F, &
                   fid,errcode,access_prp=plistmpi_id)
   dims = (/nx0G, ny0G, nz0G, frames/)

!..Create a 4D (3Dspace+time) dataspace in new file
   call h5screate_simple_f(4,dims,dspaceid,errcode)
...
!..Close created file. Each output call opens and closes the file
   ...
   end subroutine createh5_3D
!*********************************************************
!..Output 3D data to subset of 4D dataspace in dataname.h5
   subroutine outh5_3D(dataname,outdata,prec,toff)
   implicit none
   character(len=*), intent(in) :: dataname
   integer, intent(in) :: toff, prec
   real*8, intent(in) :: outdata(:,:,:)
...
!..Open file
   call h5fopen_f(trim(dirname)//trim(dataname)//'.h5', H5F_ACC_RDWR_F, &
                   fid, errcode, access_prp=plistmpi_id)
   dims = (/nx0G,ny0G,nz0G/)                   ! Shape of xy data
   count = (/nx0,ny0,nz0,1/)                   ! Size of hyperslab
!..Hyperslab offset
   offset(1) = xID*count(1)
   offset(2) = yID*count(2)
   offset(3) = zID*count(3)
   offset(4) = toff
...
!..Create memory dataspace in selected hyperslab
   call h5screate_simple_f(3,count(1:3),mspaceid,errcode)

!..Open dataspace/get dataspace ID
   call h5dget_space_f(dsetid,dspaceid,errcode)

!..Select subset of 4D dataspace to store 4D data
   call h5sselect_hyperslab_f(dspaceid,H5S_SELECT_SET_F,offset, &
                             count,errcode)

!..Write 3D data to 3D subset of 4D memspace in dataname.h5
   if (prec==1) then        ! single precision
!..BZ: THIS currently CORRUPTS data, at least as
!.......seen in MATLAB pcolor plots
      call h5dwrite_f(dsetid,H5T_NATIVE_REAL,SNGL(outdata),dims, &
...
subroutine outputxyz
   ...
   call outh5_3D('rho',rho(g:i,g:j,g:k),2,ntt)
   call outh5_3D('vx',vx(g:i,g:j,g:k),2,ntt)
   call outh5_3D('vy',vy(g:i,g:j,g:k),2,ntt)
   call outh5_3D('vz',vz(g:i,g:j,g:k),2,ntt)
   call outh5_3D('p',p(g:i,g:j,g:k),2,ntt)
   call outh5_3D('bx',bx(g:i,g:j,g:k),2,ntt)
   call outh5_3D('by',by(g:i,g:j,g:k),2,ntt)
   call outh5_3D('bz',bz(g:i,g:j,g:k),2,ntt)
...
!--MPI_CART boundary conditions along x, y and z. TRUE=periodic
   CART_BCs(1) = .FALSE.  ! not periodic along z
   CART_BCs(2) = .FALSE.  ! not periodic along y
...
!..Create x subcommunicators
   ...
   remain(2)=.FALSE.     ! Don't keep second (y) direction
   remain(3)=.TRUE.      ! Keep third (x) direction
...
!..Create y subcommunicators
   remain(1)=.FALSE.     ! Don't keep first (z) direction
   remain(2)=.TRUE.      ! Keep second (y) direction
   remain(3)=.FALSE.     ! Don't keep third (x) direction
...
!..Create z subcommunicators
   remain(1)=.TRUE.      ! Keep first (z) direction
   remain(2)=.FALSE.     ! Don't keep  second (y) direction
   remain(3)=.FALSE.     ! Don't keep third (x) direction
   call MPI_Cart_sub(COMM_CART, remain, COMM_Z, ierror)
   call MPI_Comm_rank(COMM_Z, zID, ierr)
   call MPI_Cart_coords(COMM_Z, zID, 1, zcoord, ierror)
!..For communication along y, save source and destination rank
   call MPI_CART_SHIFT(COMM_Z,0,1,back,front,ierror)
!..These two lines maybe needed for some clusters
   if (zID==0) back=MPI_PROC_NULL
   if (zID==zprocs-1) front=MPI_PROC_NULL
...
!..first define global grid
   allocate(x0(nx0G+1),y0(ny0G+1),z0(nz0G+1))
   allocate(xx0(nxG+1),yy0(nyG+1),zz0(nzG+1))
...
!..define ghost cells
   forall(i=1:ng)
      ...
      zz0(i)=2.*z0(1)-z0(ng+2-i)
      zz0(nzG+2-i)=2.*z0(nz0G+1)-z0(nz0G-ng+i)
   endforall  
...
!..now, define local grid
   ...
   forall(k=1:nz+1) zblock(:,:,k)=zz0(k+zID*nz0)
...
!.. instead of in the dealloc_par subroutine to save memory
   deallocate(x,y,z)
!   deallocate(dx,dy,dz,xi,yi,zi,xj,yj,zj,xk,yk,zk)
...
subroutine step
   ...
   elseif(stepop.eq.3) then
      ...
      print*,'Undefined time adavnce option!'
      stop
   endif
